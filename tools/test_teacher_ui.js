...
ok_('…sat between OT รับช้า and สรุปรายชั้นเรียน',
  /A_studentOT\(\)'\][\s\S]{0,600}A_editAttPick\(\)'\][\s\S]{0,160}A_studentReport\(\)'\]/.test(app));
ok_('…with the new attendance check between the OT list and the correction tool',
  /A_studentOT\(\)'\][\s\S]{0,440}A_attAudit\(\)'\][\s\S]{0,160}A_editAttPick\(\)'\]/.test(app));
/* v407 put งดคำนวณ OT directly after the OT list — it is about OT, it is used on the day, and it is
 * the screen an admin wants beside the charges it suppresses. The order is still
 * OT → waive → check → correct → report, which is the order of the job. */
ok_('…and งดคำนวณ OT sits with the OT list it acts on',
  /A_studentOT\(\)'\][\s\S]{0,200}A_otWaive\(\)'\][\s\S]{0,200}A_attAudit\(\)'\]/.test(app));
